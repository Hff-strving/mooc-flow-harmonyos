...
- 课程发现：首页分类筛选 + 搜索 + 课程列表（来自 `entry/src/main/resources/rawfile/mooc_data.json`）
...
- 课程封面/图标：`entry/src/main/resources/base/media/`（以及项目中实际使用的资源目录）
...
数据库文件位于应用沙箱（包名见 `AppScope/app.json5`），常见路径如下：
...
- 配置 SDK 环境变量（示例）：`DEVECO_SDK_HOME=E:\DevEco Studio\sdk`
- 运行 hvigor：`E:\DevEco Studio\tools\hvigor\bin\hvigorw.bat -p entry assembleHap --no-daemon`
